...
- Ordered task sequence, any parallel hints for execution surfaces that support
  them, `$wyrd-implement` invocation contract, and the conditions that
  return work to plan-level authority.
...
- [ ] Each task's `$wyrd-implement` skill accepts its write set and can execute
      without a material design choice.
